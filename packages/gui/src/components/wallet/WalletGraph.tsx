...
// import { Flex, FormatLargeNumber } from '@renamemelite/core';
...
function prepareGraphPoints(
  ...
  */
  ...
  const points = [
    {
      x: peak.height,
      y: Math.max(0, mojo_to_renamemelite(start)),
      tooltip: mojo_to_renamemelite(balance),
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: Math.max(0, mojo_to_renamemelite(start)),
      tooltip: mojo_to_renamemelite(start),
    });
  });

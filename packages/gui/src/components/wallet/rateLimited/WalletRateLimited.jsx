...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const flaxlightper_input = ip_parsed.limit;
    ...
    const flaxlightper_value = Number.parseInt(Number(flaxlightper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (flaxlight per interval):{' '}
                  {mojo_to_flaxlight_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = flaxlight_to_mojo(fee_input.value);

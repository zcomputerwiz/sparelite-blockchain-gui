...
// global.daemon_rpc_ws = `wss://${self_hostname}:56602`;
...
async function loadConfig(net) {
  try {
    // check if SPARE_ROOT is set. it overrides 'net'
    const config_root_dir =
      'SPARE_ROOT' in process.env
        ? process.env.SPARE_ROOT
        : path.join(os.homedir(), '.sparelite', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 56602); // jshint ignore:line

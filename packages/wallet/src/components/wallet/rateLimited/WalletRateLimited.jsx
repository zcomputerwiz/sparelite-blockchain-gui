...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const spareliteper_input = ip_parsed.limit;
    ...
    const spareliteper_value = Number.parseInt(Number(spareliteper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (sparelite per interval):{' '}
                  {graviton_to_sparelite_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = sparelite_to_graviton(fee_input.value);

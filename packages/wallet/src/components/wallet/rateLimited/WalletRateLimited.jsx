...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const renamemeliteper_input = ip_parsed.limit;
    ...
    const renamemeliteper_value = Number.parseInt(Number(renamemeliteper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (renamemelite per interval):{' '}
                  {mojo_to_renamemelite_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = renamemelite_to_mojo(fee_input.value);
